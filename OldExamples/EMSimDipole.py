...
def J_Dipole(dx, dt, N, x0, c, mu0):
    J = np.zeros((np.prod(N), 4))
    J[int(N[0] / 4) + int(N[1] / 2) * N[0] + int(N[2] / 2) * N[0] * N[1], 0] = 1 / np.prod(dx)
    J[int(3 * N[0] / 4) + int(N[1] / 2) * N[0] + int(N[2] / 2) * N[0] * N[1], 0] = -1 / np.prod(dx)
...
Sim = EM.sim(dx, 1, N, J = J_Dipole, approx_n = base_approx_n * Size, x0 = x0)
