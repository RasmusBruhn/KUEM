...
def J_LineCurrent(dx, dt, N, x0, c, mu0):
    ...
    J[int(N[0] / 2) + int(N[1] / 2) * N[0] :: N[0] * N[1], 3] = 1 / (dx[0] * dx[1])
...
Sim = EM.sim(dx, 1, N, J = J_LineCurrent, approx_n = base_approx_n * Size, x0 = x0)
...
Sim.plot_B(x, axis1 = x[:, :, 0], axis2 = x[:, :, 1])
Sim.plot_AComp(x, 3, scale = EM.optional_scale_id)
