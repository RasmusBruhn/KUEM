...
def ConvergenceVolumeTest(J, k, N_vals, Threshold, n_max = 1000):
    ...
    for i in range(N_vals.shape[0]):
        ...
        for n in range(n_max):
            if np.mean(np.abs((A - Exact))) <= np.sqrt(Threshold):#np.sum((A - Exact) ** 2) / np.prod(N_use) <= Threshold:
                n_use = n
                break
...
delta_x = np.array([1, 1, 1], dtype = int)
dx = delta_x / (N + 1)
Charge = 1
...
Vals2, Cov2 = opt.curve_fit(Fit3, N_finallist, Values2, maxfev = 5000)

plt.figure()
plt.plot(N_finallist[(N_finallist[:, 0] == 5) & (N_finallist[:, 1] == 5), 2], Values2[(N_finallist[:, 0] == 5) & (N_finallist[:, 1] == 5)], "-", label = "Data")
plt.plot(N_finallist[(N_finallist[:, 0] == 5) & (N_finallist[:, 1] == 5), 2], Fit3(N_finallist[(N_finallist[:, 0] == 5) & (N_finallist[:, 1] == 5), :], *Vals2), "-", label = "Fit")
plt.legend()
plt.figure()
plt.plot(N_finallist[(N_finallist[:, 0] == 10) & (N_finallist[:, 1] == 10), 2], Values2[(N_finallist[:, 0] == 10) & (N_finallist[:, 1] == 10)], "-", label = "Data")
plt.plot(N_finallist[(N_finallist[:, 0] == 10) & (N_finallist[:, 1] == 10), 2], Fit3(N_finallist[(N_finallist[:, 0] == 10) & (N_finallist[:, 1] == 10), :], *Vals2), "-", label = "Fit")
plt.legend()
plt.figure()
plt.plot(N_finallist[(N_finallist[:, 0] == 15) & (N_finallist[:, 1] == 15), 2], Values2[(N_finallist[:, 0] == 15) & (N_finallist[:, 1] == 15)], "-", label = "Data")
plt.plot(N_finallist[(N_finallist[:, 0] == 15) & (N_finallist[:, 1] == 15), 2], Fit3(N_finallist[(N_finallist[:, 0] == 15) & (N_finallist[:, 1] == 15), :], *Vals2), "-", label = "Fit")
plt.legend()
#plt.figure()
