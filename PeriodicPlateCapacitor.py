...
SurfaceCharge = 1
Size = np.array([1, 1, 10000], dtype = int)
approx_n = 10
exact = True
x0 = np.array([0, 0, -1], dtype = float)
delta_x = np.array([1, 1, 2], dtype = float)

# Create J function
d = 0.3
def J(dx, N, x0, c, mu0):
    J_Array = np.zeros(tuple(N) + (4,))
    J_Array[:, :, int(N[2] * (1 - d) / 2), 0] = SurfaceCharge / dx[2]
    J_Array[:, :, int(N[2] * (1 + d) / 2), 0] = -SurfaceCharge / dx[2]
...
Sim = EM.sim(Size, delta_x = delta_x, x0 = x0, approx_n = approx_n, approx_k = 1, J = J, boundaries = ["periodic", "periodic", ["closed", "flat"]])
...
# Plot E in xy plane
EM.plot_vector(vx3, vy3, extent = [Center[0] - Width[0] / 2, Center[0] + Width[0] / 2, Center[1] - Width[1] / 2, Center[1] + Width[1] / 2], cutoff = 0.001)
